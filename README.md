...
| `npm run deploy`                    | Production-сборка + публикация `build/` на GitHub Pages (ветка `docs`) |
...
> **Lint не входит в `npm run build` и `npm run dev`.** Проверка стилей и скриптов
> запускается отдельно — это сознательное решение, чтобы сборка не падала из-за
> стилистических замечаний в библиотечных блоках шаблона.
...
│   ├── deploy.mjs             # Деплой build/ на GitHub Pages
...
Команда `npm run deploy` публикует содержимое `build/` на GitHub Pages.
Сторонние пакеты не нужны — используется только git.

**Что происходит:**

1. Production-сборка (`npm run build`)
2. Содержимое `build/` выделяется через `git subtree split`
3. Результат пушится в ветку `docs` (`git push --force`)
...
Settings → Pages → Source: ветка `docs`, папка `/ (root)`.

> Временный deploy-коммит создаётся локально и откатывается после push —
> каталог `build/` не попадает в историю рабочей ветки.
